...
// Consecutive damaged frames tolerated before flushing and waiting for a fresh keyframe.
// Interpolated between these two by measured source framerate in `record_damage`, matching
// the low- and high-framerate limits below.
...
pub struct VideoRtp {
    ...
    /// the server changed encode resolution mid-session, which the hardware decoder can't
    /// follow without being recreated first.
    decode_width: u32,
    decode_height: u32,
...
impl VideoRtp {
    ...
    pub fn receive(
        ...
        if sample_too_large {
            ...
            // frames that the Vita hardware cannot decode. A flush, not a decoder rebuild - see
            // `VideoDecodeWorker::begin_resync`.
            *keyframe_requested = true;
    ...
    fn record_damage(&mut self, worker: &VideoDecodeWorker) {
        ...
        // Scaled by the *measured* source framerate: at 60 fps a corrupted
        // reference frame poisons twice as many following frames per second, so resync sooner.
        let source_fps = self
        ...
        // A flush, never a decoder rebuild - see `VideoDecodeWorker::begin_resync` for why this
        // distinction is what separates 8 fps from 60 on a lossy link.
        worker.begin_resync();
        self.waiting_for_keyframe = true;
        self.damage_score = 0;
    }
